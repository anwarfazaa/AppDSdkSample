...
/**
 * The maximum size of a data segment in a single analytics transaction, in bytes.
 * Larger segments will be dropped.
 *
 * @param cfg A pointer to the default application context structure created by appd_config_init().
 * @param size An unsigned int indicating the maximum size of data segment.
 * @return void
 */
APPD_API void appd_config_set_max_segment_size_in_bytes(struct appd_config* cfg,
                                                        unsigned int size);

/**
 * The maximum number of data segments in a single analytics transaction.
 * Each segment over the limit will be dropped.
 *
 * @param cfg A pointer to the default application context structure created by appd_config_init().
 * @param size An unsigned int indicating the maximum number of data segments.
 * @return void
 */
APPD_API void appd_config_set_max_segments_per_request(struct appd_config* cfg,
                                                       unsigned int num);
/**
 * The maximum total size of data segments in a single analytics transaction, in bytes.
 * Each segment resulting in exceeding the limit will be dropped.
 *
 * @param cfg A pointer to the default application context structure created by appd_config_init().
 * @param size An unsigned int indicating the maximum total size of data segments.
 * @return void
 */
APPD_API void appd_config_set_max_message_size_in_bytes(struct appd_config* cfg,
                                                        unsigned int size);
